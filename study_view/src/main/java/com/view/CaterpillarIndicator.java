 ...
 * @创建日期 2019/7/8 11:43
 */
public class CaterpillarIndicator extends LinearLayout implements android.view.View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private java.util.List<TitleInfo> mTitles;
...
//            Log.e(TAG, "mCurrentScroll  : " + mCurrentScroll + "scroll_x" + scroll_x);
//            int scrollx = mSelectedTab > 0 ? mCurrentScroll / mSelectedTab : mCurrentScroll;
            if (scroll_x < 0) {
                int scrollx = mSelectedTab + 1 % mCurrentScroll;
                ...
                android.util.Log.e(TAG, "viewChildLeft  : " + viewChildLeft + "getWidth()" + getWidth() + "mCurrentScroll" + mCurrentScroll);
                leftX = viewChildLeft - (getWidth() - scrollx);
            } else if (scroll_x > 0) {
//                int scrollx = mSelectedTab + 1 % mCurrentScroll;
                int scrollx = mCurrentScroll % (getWidth() / mItemCount);
                int viewChildRight = getViewChildRight(mSelectedTab);
                int nextChildRight = getViewChildRight(mSelectedTab + 1);
                leftX = getViewChildLeft(mSelectedTab);
                rightX = viewChildRight + scrollx;
                android.util.Log.e(TAG, "origin rightX " + rightX + "origin leftX " + leftX);
                if (rightX > nextChildRight) {
                    rightX = nextChildRight;
                    leftX = leftX + scrollx - (nextChildRight - viewChildRight);
                }
                android.util.Log.e(TAG, "rightX " + rightX + "leftX " + leftX);
            } else {
                // 区分第一个子View的区别，无需add
                if (mSelectedTab > 0) {
                    int itemWidthAll = 0;
     ...
     */
    public void init(int startPosition, java.util.List<TitleInfo> tabs, ViewPager mViewPager) {
        removeAllViews();
     ...
     */
    public void init(int startPosition, java.util.List<TitleInfo> tabs, ViewPager mViewPager, boolean hasPic) {
        removeAllViews();
        this.mSelectedTab = startPosition;
        this.mViewPager = mViewPager;
        this.mViewPager.addOnPageChangeListener(this);
    ...
    protected void add(String label, int position, int drawable) {
        ...
        if (drawable != 0) {
            android.graphics.drawable.Drawable drawable_myrecord = getResources().getDrawable(
                    drawable);
    ...
    public synchronized void setCurrentTab(int index) {
        ...
        android.view.View oldTab = getChildAt(mSelectedTab);
        if (oldTab != null) {
        ...
        mSelectedTab = index;
        android.view.View newTab = getChildAt(mSelectedTab);
        if (newTab != null) {
     ...
     */
    private void setTabTextSize(android.view.View tab, boolean selected) {
        if (tab instanceof android.widget.TextView) {
            android.widget.TextView tv = (android.widget.TextView) tab;
